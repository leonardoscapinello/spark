...
export function createLeadFormsCollection() { return createCollection(electricCollectionOptions({ id: "lead_forms", schema: LeadFormSchema, getKey: (item) => item.id, shapeOptions: shape("lead_forms") })); }
export function createFormSubmissionsCollection() { return createCollection(electricCollectionOptions({ id: "form_submissions", schema: FormSubmissionSchema, getKey: (item) => item.id, shapeOptions: shape("form_submissions") })); }
