...
export function optimisticConversation(input: Omit<CreateConversationInput, "id">, orgId: OrgId, assigneeId: Conversation["assigneeId"]): Conversation {
  ...
  return { id: conversationId.create(), orgId, contactId: input.contactId, channel: input.channel, subject: input.subject, status: "open", priority: "normal", assigneeId, teamId: null, snoozedUntil: null, firstResponseDueAt: firstResponseDueAt(now, "normal"), firstRespondedAt: null, resolvedAt: null, lastMessageAt: now, createdAt: now, updatedAt: now };
}
