...
export function createProductsCollection() { return createCollection(electricCollectionOptions({ id: "products", schema: SyncedProductSchema, getKey: (item) => item.id, shapeOptions: options("products") })); }
export function createProductVariantsCollection() { return createCollection(electricCollectionOptions({ id: "product_variants", schema: SyncedVariantSchema, getKey: (item) => item.id, shapeOptions: options("product_variants") })); }
export function createDiscountRulesCollection() { return createCollection(electricCollectionOptions({ id: "discount_rules", schema: SyncedDiscountSchema, getKey: (item) => item.id, shapeOptions: options("discount_rules") })); }
