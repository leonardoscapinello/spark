 ...
 */
export function StageProgress<Id extends string = string>({ stages, currentId, durations, details, outcome, onSelect, onMove, interaction = "popover", label = "Etapas do funil" }: StageProgressProps<Id>) {
      ...
      return <li key={stage.id} className={s.step} data-state={state}>
        {interactive && interaction === "popover" ? <Popover open={openId === stage.id} onOpenChange={(open) => { setError(null); setExpandedId(null); setOpenId(open ? stage.id : null); }}>
          ...
          <PopoverContent title={stage.label} className={s.stagePopover} {...(s.stagePopoverContent ? { contentClassName: s.stagePopoverContent } : {})}>
            ...
            {onMove && state !== "current" && <Button loading={pendingId === stage.id} onClick={() => { setPendingId(stage.id); setError(null); Promise.resolve(onMove(stage.id)).then(() => setOpenId(null)).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "Não foi possível mudar a etapa.")).finally(() => setPendingId(null)); }}>Mudar para esta etapa</Button>}
          </PopoverContent>
