...
export function DataTable<T>({label,rows,columns,rowKey,actions,renderExpanded,rowLabel=rowKey,state="ready",onRetry,emptyText="Nenhum registro encontrado"}:DataTableProps<T>){
 ...
 return <div className={s.root}>{state==="loading"?<div className={s.scroll} role="status" aria-label={`Carregando ${label}`}><table className={s.table} aria-hidden="true"><caption>{label}</caption><thead><tr>{columns.map(column=><th key={column.id} scope="col">{column.label}</th>)}{hasActions&&<th scope="col"><span className={s.visuallyHidden}>Ações</span></th>}</tr></thead><tbody>{[0,1,2].map(index=><tr key={index}>{columns.map(column=><td key={column.id}><span className={s.loadingLine}/></td>)}{hasActions&&<td><span className={s.loadingLine}/></td>}</tr>)}</tbody></table></div>:state==="error"?<div role="alert"><p>Não foi possível carregar os registros.</p>{onRetry&&<Button variant="secondary" onClick={onRetry}>Tentar novamente</Button>}</div>:<><div ref={scrollRef} className={s.scroll} role="region" aria-label={label} tabIndex={0}><table className={s.table}><caption>{label}</caption><thead><tr>{columns.map(column=><th key={column.id} scope="col" data-align={column.align} aria-sort={column.sortValue?(sort?.id===column.id?(sort.direction===1?"ascending":"descending"):"none"):undefined}>{column.sortValue?<Button size="sm" variant="ghost" className={s.sortButton} onClick={()=>setSort(previous=>({id:column.id,direction:previous?.id===column.id&&previous.direction===1?-1:1}))}>{column.label}{sort?.id===column.id&&<Icon name={sort.direction===1?"up":"chevron"} className={s.sortIcon}/>}</Button>:column.label}</th>)}{hasActions&&<th scope="col"><span className={s.visuallyHidden}>Ações</span></th>}</tr></thead><tbody>{ordered.map(row=><Fragment key={rowKey(row)}><tr>{columns.map(column=><td key={column.id} data-align={column.align}>{column.cell(row)}</td>)}{hasActions&&<td className={s.actions}><TableActions label={`Ações de ${rowLabel(row)}`}>{actions?.(row)}{renderExpanded&&<TableIconAction label={`${expanded.has(rowKey(row))?"Recolher":"Expandir"} detalhes de ${rowLabel(row)}`} aria-expanded={expanded.has(rowKey(row))} aria-controls={`${detailId}-${encodeURIComponent(rowKey(row))}`} icon={<Icon name={expanded.has(rowKey(row))?"up":"chevron"}/>} onClick={()=>setExpanded(previous=>{const next=new Set(previous);if(next.has(rowKey(row)))next.delete(rowKey(row));else next.add(rowKey(row));return next;})}/>}</TableActions></td>}</tr>{renderExpanded&&<tr hidden={!expanded.has(rowKey(row))} id={`${detailId}-${encodeURIComponent(rowKey(row))}`}><td colSpan={columns.length+1}>{expanded.has(rowKey(row))&&renderExpanded(row)}</td></tr>}</Fragment>)}</tbody></table></div>{ordered.length===0&&<div className={s.emptyState} role="status">{emptyText}</div>}{hasOverflow&&ordered.length>0&&<p className={s.hint}>Deslize a tabela para ver todas as colunas.</p>}</>}</div>;
}
