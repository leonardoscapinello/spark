...
export interface DataTableProps<T> {label:string;rows:readonly T[];columns:readonly TableColumn<T>[];rowKey:(row:T)=>string;actions?:(row:T)=>ReactNode;renderExpanded?:(row:T)=>ReactNode;rowLabel?:(row:T)=>string;state?:"ready"|"loading"|"error";onRetry?:()=>void;emptyText?:string;selectedIds?:readonly string[];onSelectionChange?:(ids:string[])=>void;hiddenColumnIds?:readonly string[];onHiddenColumnsChange?:(ids:string[])=>void;columnWidths?:Readonly<Record<string,number>>;onColumnWidthsChange?:(widths:Record<string,number>)=>void}
export function DataTable<T>({label,rows,columns,rowKey,actions,renderExpanded,rowLabel=rowKey,state="ready",onRetry,emptyText="Nenhum registro encontrado",selectedIds,onSelectionChange,hiddenColumnIds,onHiddenColumnsChange,columnWidths,onColumnWidthsChange}:DataTableProps<T>){
