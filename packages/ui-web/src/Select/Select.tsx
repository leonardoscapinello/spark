...
export function Select<Multiple extends boolean = false>({ options, label, placeholder = "Selecionar", ...props }: SelectProps<Multiple>) {
  return <BaseSelect.Root<string, Multiple> items={options} {...props}>
    <BaseSelect.Trigger className={s.trigger} aria-label={label}><BaseSelect.Value className={s.selectValue} placeholder={placeholder} /><BaseSelect.Icon className={s.triggerIcon}><Icon name="chevron" /></BaseSelect.Icon></BaseSelect.Trigger>
    <BaseSelect.Portal><BaseSelect.Positioner alignItemWithTrigger={false} sideOffset={Number.parseFloat(lightTheme["space-1"])} className={s.positioner}><BaseSelect.Popup className={`${s.popup} ${s.selectPopup}`}><BaseSelect.List>
      {options.map(o => <BaseSelect.Item key={o.value} value={o.value} disabled={o.disabled ?? false} className={s.item}><span className={s.indicator}><BaseSelect.ItemIndicator><Icon name="check" /></BaseSelect.ItemIndicator></span><BaseSelect.ItemText>{o.label}</BaseSelect.ItemText></BaseSelect.Item>)}
