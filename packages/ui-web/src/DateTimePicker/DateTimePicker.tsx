...
export function DateTimePicker({label,value,onValueChange,mode="date",disabled=false,placeholder="Selecionar"}:DateTimePickerProps){
 ...
 {mode!=="date" && <div className={s.time}><Clock time={time}/><div className={s.timeFields}><Select label="Horas" value={hour} onValueChange={v=>{if(v)setTime(`${v}:${minute}`);}} options={Array.from({length:24},(_,n)=>({value:pad(n),label:pad(n)}))}/><span aria-hidden="true">:</span><Select label="Minutos" value={minute} onValueChange={v=>{if(v)setTime(`${hour}:${v}`);}} options={Array.from({length:60},(_,n)=>({value:pad(n),label:pad(n)}))}/></div><p className={s.hint}>Formato de 24 horas</p></div>}
