...
5. ~~Consolidar o inventário em [`../especificacao/componentes-intercom.md`](../especificacao/componentes-intercom.md)~~ — feito em 13/09: a conferência contra o código está na seção «Consolidação do inventário». Restam ali, como faltando de fato, **editor rico**, **ícones exatos** e **hover/foco medidos sistematicamente**.

## Retomada de 13/09

A linha de tabela foi fechada por inteiro no `DataTable`: **seleção de linhas**, **catálogo de colunas**, **largura de coluna** e **reordenação de coluna**. Todas controladas pela tela — o componente informa o que mudou e não guarda preferência — e cobertas por teste e história no Storybook. Arrasto e largura foram conferidos também no navegador, não só em jsdom. Também foi corrigida uma regressão em que o formulário de edição de pessoa ficava sem estilo — o módulo CSS não tinha as classes que a tela referenciava.
