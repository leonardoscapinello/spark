...
import { auditLogs, createDbClient, permissionGroups, users, withOrgContext, type SparkDb } from "@spark/db";
...
export class AuditLogsRepository {
  ...
  async listByOrg(orgId: OrgId): Promise<AdminAuditLog[]> {
    return withOrgContext(this.db, orgId, async (tx) => {
      ...
      const [actors, targetUsers, targetGroups] = await Promise.all([
      ...
      const targetNames = new Map([...targetUsers, ...targetGroups].map((target) => [target.id, target.name]));
      ...
      return logs.map((log) => ({
        ...
        targetType: log.targetType as "permission_group" | "user",
