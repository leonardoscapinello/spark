...
export class WhatsAppWebhookController {
  constructor(private readonly webhooks: WhatsAppWebhookRepository, private readonly queue: WebhookQueue) {}
  ...
  async receive(@Param("connectionId") rawId: string, @Req() request: RawBodyRequest<FastifyRequest>, @Body() payload: unknown): Promise<{ received: true }> {
    ...
    await this.queue.enqueue({ provider: "whatsapp", connectionId: rawId, payload });
