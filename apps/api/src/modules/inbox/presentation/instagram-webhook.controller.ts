...
export class InstagramWebhookController {
  constructor(private readonly webhooks: InstagramWebhookRepository) {}
  ...
  async receive(@Param("connectionId") rawId: string, @Req() request: RawBodyRequest<FastifyRequest>, @Body() payload: unknown): Promise<{ received: true; inserted: number }> {
    ...
    const inserted = await this.webhooks.receive(connection.orgId, payload, connection.accountId);
