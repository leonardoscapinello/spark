...
import type { AddInternalNoteInput, Conversation, ConversationId, ConversationWriteResponse, CreateConversationInput, Message, MessageWriteResponse, OrgId, UpdateConversationInput, UserId } from "@spark/core";
...
function toConversation(row: typeof conversations.$inferSelect): Conversation {
  return { ...row, snoozedUntil: row.snoozedUntil?.toISOString() ?? null, lastMessageAt: row.lastMessageAt.toISOString(), createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() } as Conversation;
}
