...
import { contactId, conversationId, firstResponseDueAt, identityId, messageId, normalizeIdentityValue, parseTelegramInboundTexts, type IntegrationConnectionId, type OrgId } from "@spark/core";
...
export class TelegramWebhookRepository {
  ...
  async connection(id: IntegrationConnectionId): Promise<{ orgId: OrgId; secretToken: string }> {
    ...
    return { orgId, secretToken: secrets.secretToken };
  }

  async receive(orgId: OrgId, payload: unknown): Promise<number> {
    const incoming = parseTelegramInboundTexts(payload);
    let inserted = 0;
    for (const item of incoming) {
      const added = await withOrgContext(this.db, orgId, async (tx) => {
        const externalValue = normalizeIdentityValue("telegram", item.senderId);
        ...
        if (!existing) {
          await tx.insert(conversations).values({ id: threadId, orgId, contactId: leadId, channel: "telegram", subject: "Telegram", firstResponseDueAt: new Date(firstResponseDueAt(item.occurredAt, "normal")), lastMessageAt: item.occurredAt, createdAt: item.occurredAt });
        ...
        await this.events.append(tx, { orgId, contactId: leadId as ReturnType<typeof contactId.create>, type: "message.received", data: { conversationId: threadId, messageId: message.id, channel: "telegram" } });
        return true;
      });
      if (added) inserted += 1;
    }
