...
export class OutboundMessagesRepository {
  ...
  async send(orgId: OrgId, actorUserId: UserId, conversationId: ConversationId, input: SendMessageInput): Promise<MessageWriteResponse> {
    const queued = await withOrgContext(this.db, orgId, async (tx) => {
      ...
      const now = new Date();
      const [message] = await tx.insert(messages).values({ id: input.id, orgId, conversationId, contactId: conversation.contactId, authorUserId: actorUserId, direction: "outbound", status: "queued", body: input.body, createdAt: now }).returning();
    ...
    try {
      const externalId = await this.sender.send(orgId, queued.contactId as Message["contactId"], queued.channel as Conversation["channel"], queued.subject, input.body);
...
function toMessage(row: typeof messages.$inferSelect): Message { return { ...row, createdAt: row.createdAt.toISOString() } as Message; }
function toConversation(row: typeof conversations.$inferSelect): Conversation { return { ...row, snoozedUntil: row.snoozedUntil?.toISOString() ?? null, firstResponseDueAt: row.firstResponseDueAt.toISOString(), firstRespondedAt: row.firstRespondedAt?.toISOString() ?? null, lastMessageAt: row.lastMessageAt.toISOString(), createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() } as Conversation; }
