...
export class ChannelSender {
  ...
  async send(orgId: OrgId, contactId: ContactId, channel: ConversationChannel, subject: string, body: string): Promise<string> {
    if (channel !== "email" && channel !== "instagram" && channel !== "whatsapp" && channel !== "messenger") throw new BadRequestException(`O canal ${channel} ainda não aceita respostas externas.`);
