...
export class ChannelSender {
  ...
  async send(orgId: OrgId, contactId: ContactId, channel: ConversationChannel, subject: string, body: string): Promise<string> {
    ...
    if (loaded.connection.provider === "telegram") return this.telegram(loaded.credentials, loaded.recipient, body);
    if (loaded.connection.provider === "messenger") return this.metaSend(text(loaded.config.pageId), text(loaded.config.apiVersion), loaded.credentials, loaded.recipient, body, "Messenger");
    return this.metaSend(text(loaded.config.accountId), text(loaded.config.apiVersion), loaded.credentials, loaded.recipient, body, "Instagram");
  }
  private async telegram(credentials: Record<string, string>, recipient: string, body: string): Promise<string> { if (!credentials.botToken) throw new ServiceUnavailableException("Token do Telegram indisponível."); const response = await fetch(`https://api.telegram.org/bot${credentials.botToken}/sendMessage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: recipient, text: body }) }); const payload = await json(response); if (!response.ok) throw new Error(apiError(payload, "O Telegram recusou a mensagem.")); const result = payload.result; return result && typeof result === "object" && !Array.isArray(result) && typeof (result as Record<string, unknown>).message_id !== "undefined" ? String((result as Record<string, unknown>).message_id) : crypto.randomUUID(); }
  private async metaSend(pageOrAccountId: string, apiVersion: string, credentials: Record<string, string>, recipient: string, body: string, label: string): Promise<string> { const version = apiVersion || "v23.0"; if (!pageOrAccountId || !credentials.accessToken) throw new ServiceUnavailableException(`Conta ou token do ${label} indisponível.`); const response = await fetch(`https://graph.facebook.com/${version}/${encodeURIComponent(pageOrAccountId)}/messages`, { method: "POST", headers: { Authorization: `Bearer ${credentials.accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ recipient: { id: recipient }, messaging_type: "RESPONSE", message: { text: body } }) }); const payload = await json(response); if (!response.ok) throw new Error(apiError(payload, `O ${label} recusou a mensagem.`)); return text(payload.message_id) || text(payload.id) || crypto.randomUUID(); }
