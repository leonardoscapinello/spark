...
import { contactId, conversationId, firstResponseDueAt, identityId, messageId, normalizeIdentityValue, parseWhatsAppInboundTexts, type IntegrationConnectionId, type OrgId } from "@spark/core";
...
export class WhatsAppWebhookRepository {
  ...
  async connection(id: IntegrationConnectionId): Promise<{ orgId: OrgId; appSecret: string; verifyToken: string; phoneNumberId?: string }> {
    ...
    return { orgId, appSecret: secrets.appSecret, verifyToken: secrets.verifyToken, ...(phoneNumberId ? { phoneNumberId } : {}) };
  }

  async receive(orgId: OrgId, payload: unknown, phoneNumberId?: string): Promise<number> {
    const incoming = parseWhatsAppInboundTexts(payload, phoneNumberId);
    let inserted = 0;
    for (const item of incoming) {
      const added = await withOrgContext(this.db, orgId, async (tx) => {
        const externalValue = normalizeIdentityValue("whatsapp", item.senderId);
        ...
        if (!existing) {
          await tx.insert(conversations).values({ id: threadId, orgId, contactId: leadId, channel: "whatsapp", subject: "WhatsApp", firstResponseDueAt: new Date(firstResponseDueAt(item.occurredAt, "normal")), lastMessageAt: item.occurredAt, createdAt: item.occurredAt });
        ...
        await this.events.append(tx, { orgId, contactId: leadId as ReturnType<typeof contactId.create>, type: "message.received", data: { conversationId: threadId, messageId: message.id, channel: "whatsapp" } });
        return true;
      });
      if (added) inserted += 1;
    }
