...
export default function Automations() {
  ...
  return <PageFrame>
    ...
    {!firstRun && <><CollectionToolbar search={<Input aria-label="Buscar automações" startAdornment={<Icon name="search" />} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar automação" />} filters={<Select appearance="filter" label="Filtrar automações por gatilho" value={triggerFilter} options={[{ value: "all", label: "Qualquer gatilho" }, ...triggerOptions.map((label) => ({ value: label, label }))]} onValueChange={(value) => setTriggerFilter(value ?? "all")} />} count={initialLoad ? "Carregando…" : `${visibleAutomations.length} ${visibleAutomations.length === 1 ? "automação" : "automações"}`} actions={<ViewSwitcher label="Visualização das automações" value={layout} onValueChange={setLayout} />} />
