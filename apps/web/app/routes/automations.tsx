...
export default function Automations() {
  ...
  return <PageFrame width="content">
    ...
    {firstRun && <EmptyState variant="featured" icon="bolt" title="Crie sua primeira automação" description="Comece por um gatilho, escolha o que deve acontecer e acompanhe cada execução no mesmo fluxo." />}
    {firstRun && (canWrite || canReadContacts || canReadIntegrations) && <ActionCardGroup title="Prepare seu primeiro fluxo">
      {canWrite && <ActionCard icon="bolt" title="Desenhe o fluxo" description="Crie uma automação e escolha os gatilhos e ações no editor." action={<Button variant="secondary" onClick={() => setModalOpen(true)}>Nova automação</Button>} />}
      {canReadContacts && <ActionCard icon="team" title="Veja seus contatos" description="Encontre as pessoas que poderão entrar nos seus fluxos." action={<Button variant="secondary" onClick={() => void navigate("/")}>Abrir contatos</Button>} />}
    ...
    {!firstRun && <><CollectionToolbar search={<Input aria-label="Buscar automações" startAdornment={<Icon name="search" />} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar automação" />} filters={<Select appearance="filter" label="Filtrar automações por gatilho" value={triggerFilter} options={[{ value: "all", label: "Qualquer gatilho" }, ...triggerOptions.map((label) => ({ value: label, label }))]} onValueChange={(value) => setTriggerFilter(value ?? "all")} />} count={`${visibleAutomations.length} ${visibleAutomations.length === 1 ? "automação" : "automações"}`} actions={<ViewSwitcher label="Visualização das automações" value={layout} onValueChange={setLayout} />} />
